...
const mkFirstFunc = (method) => (str) =>
  str.slice(0, 1)[method]() + str.slice(1);
const lowerFirst = mkFirstFunc('toLowerCase');
const capFirst = mkFirstFunc('toUpperCase');
const toCallbackName = (prop) =>
  `on${prop === 'value' ? '' : capFirst(prop)}Change`;
...
export default function controllable(...args) {
  ...
  let { reducers, initialize = noOpInitialize } =
    createStateManager(propsOrStateManager);
  ...
  return class ControllableWrapper extends React.Component {
    constructor(...args) {
      ...
      const instanceInitialState = mapKeys(
        pickDefaults(this.props),
        fromDefaultName,
      );
      ...
      this.boundActionCreators = _.mapValues(actionCreators, (fn) =>
        fn.bind(this),
      );
    }
